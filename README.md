...
`Bus` implements typed surfaces over the same event map. Compose them when a dependency needs only part of the API.

| Surface | Methods | Use when |
|---------|---------|----------|
| **`EmittingSurface`** | `emit` | Raising events without teardown |
...
`ControlSurface` extends `EmittingSurface`. Prefer `EmittingSurface` when a dependency may raise events but must not destroy the bus.
...
Brand only the surfaces the object actually exposes. Separate brands (`@@StrongbusSubscriptionSurface`, `@@StrongbusEmittingSurface`, …) keep partial exposure sound — branding a subscription surface does not claim control or monitoring.
...
const bus = new Bus<Events>({
  ...
  thresholds: {warn: 50},
  // logger omitted → built-in console adapter (record.message → console)
